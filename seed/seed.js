...
async function seed() {
  ...
  // ── GUARDA DESTRUCTIVA ────────────────────────────────────────────────────────
  // Este seed BORRA todas las colecciones antes de crear datos demo. Para no
  // arrasar una base con datos reales por accidente, se niega a correr contra
  // una base no vacía salvo que se pase --force explícitamente.
  ...
  if (!FORCE) {
    ...
    if (existingDocs > 0) {
      ...
      console.error('    Este seed BORRA TODO y lo reemplaza con datos demo.');
      console.error('');
      console.error('    Si de verdad quieres reemplazarla:  npm run seed -- --force');
      console.error('    Para agregar solo las páginas legales sin tocar nada más:  npm run seed:pages');
      console.error('');
...
<p>Invierte en un buen par. Las formas clásicas (ovaladas, rectangulares, tipo ojo de gato) superan las tendencias por décadas.</p>`,
  ...
  // ── 16b. Páginas legales (editables desde el admin → Pages) ───────────────────
  await Page.create([
    {
...
<p>Si cambiamos esta política, publicamos aquí la nueva versión con su fecha. Guardamos tus datos solo mientras tengas cuenta activa o la ley nos obligue a conservarlos; después se eliminan de forma segura.</p>`,
  ...
  console.log('Páginas legales creadas');
    ...
    options: {
      ...
      footer: {
        ...
        useful_link: [
          ...
          { id: 3, name: 'About Us',    value: 'about-us' },
          { id: 4, name: 'Offers',      value: 'offers' },
          { id: 5, name: 'Search',      value: 'search' },
        ],
        help_center: [
          { id: 1, name: 'My Account',          value: 'account/dashboard' },
          { id: 2, name: 'My Orders',           value: 'account/order' },
          { id: 3, name: 'Wishlist',            value: 'wishlist' },
          { id: 4, name: "FAQ's",               value: 'faq' },
          { id: 5, name: 'Contact Us',          value: 'contact-us' },
          { id: 6, name: 'TermsAndConditions',  value: 'terms-and-conditions' },
          { id: 7, name: 'PrivacyPolicy',       value: 'privacy-policy' },
        ],
      },
